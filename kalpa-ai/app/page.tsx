...
const MOCK_DATA = {
  sceneTitle: "Scene 14: The Rain-Slicked Alley",
  emotion: "Paranoia",
  mood: "Cinematic Noir",
  tone: "High Contrast",
  image: "https://images.unsplash.com/photo-1517604931442-7e0c8ed2963c?q=80&w=2070",
  analysis: "The shadows in this scene are elongated to represent the character's growing isolation. The neon lighting clashing with the darkness suggests a world that is both vibrant and deadly."
};
...
export default function Page() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#03050b] text-white overflow-hidden font-sans relative">
      {/* Background Atmosphere - SLIGHT PINK & CYAN */}
      ...
      {hasProcessed && (
        <aside className="w-72 h-full border-r border-white/5 flex flex-col bg-white/[0.01] backdrop-blur-3xl z-20 animate-in slide-in-from-left duration-700">
          ...
          <nav className="p-6">
            <p className="px-2 text-[10px] font-bold text-zinc-600 uppercase tracking-[0.3em] mb-4">Project Explorer</p>
          ...
          <div className="p-10 max-w-6xl mx-auto w-full space-y-10 animate-in fade-in slide-in-from-bottom duration-1000">
            ...
            <div className="bg-linear-to-r from-pink-500/5 to-transparent border-l-2 border-pink-500/20 p-8 rounded-r-3xl">
              <p className="text-zinc-400 font-mono text-sm italic leading-relaxed">
                {data ? `> ${data.analysis}` : "/// SYNTHESIZING NEURAL DATA..."}
              </p>
            </div>
          </div>
        )}
